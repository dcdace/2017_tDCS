function [RetOA, RetUN] = retentionTest(sID, seqCond, sIDDir)
...
RetOAavgs   = allSeqAvgs(dsearchn(allSeqAvgs(:,1),OAs'),:);
RetUNavgs   = allSeqAvgs(dsearchn(allSeqAvgs(:,1),UNs'),:);
% condition means
%[sID meanMT meanCorrMT totalErrors adjMT meanGaps]
RetOA       = nanmean(RetOAavgs(:,2:6));
RetUN       = nanmean(RetUNavgs(:,2:6));

filename    = fullfile(sIDDir, [sID '_retTest_' datetime '.mat']);
save(filename);
% get back to Matlab
clear Screen;
ShowCursor;
