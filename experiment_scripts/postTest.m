function [PostOA, PostUN] = postTest(sID, seqCond, sIDDir)
...
DrawFormattedText(win, 'Well done!', 'center', 'center', TextColor);
Screen('Flip', win);
WaitSecs(3);

Gaps        = OUTPUT((OUTPUT(:,3)==1),4);
% all sequences per condition means
PostOAavgs  = allSeqAvgs(dsearchn(allSeqAvgs(:,1),OAs'),:);
PostUNavgs  = allSeqAvgs(dsearchn(allSeqAvgs(:,1),UNs'),:);
% condition means
%[sID meanMT meanCorrMT totalErrors adjMT meanGaps]
PostOA      = nanmean(PostOAavgs(:,2:6));
PostUN      = nanmean(PostUNavgs(:,2:6));

filename    = fullfile(sIDDir, [sID '_postTest_' datetime '.mat']);
save(filename);
% get back to Matlab
clear Screen;
ShowCursor;
